...
(function (global) {
  ...
  function sha256 (input) {
    ...
    for (j = 0; j < int32s.length;) {
      ...
      d = d.slice(0, 8);
      for (i = 0; i < 64; i++) {
        const i2 = i + j; // eslint-disable-line no-unused-vars
        const w15 = w[i - 15];
        const w2 = w[i - 2], a = d[0], e = d[4]; // eslint-disable-line one-var
        const t1 =
          d[7] +
  ...
  class Feed {
    ...
    at (coordinates, quantity) {
      const xs = (coordinates.x < 0) ? this.xNeg : this.xPos;
      const ys = (coordinates.y < 0) ? this.yNeg : this.yPos;

      for (let i = xs.length; i <= Math.abs(coordinates.x); i++) {
        xs.push(sha256(xs[i - 1]));
      }
      for (let i = ys.length; i <= Math.abs(coordinates.y); i++) {
        ys.push(sha256(ys[i - 1]));
      }

      // For quantities beyond 32^2, the sequence will repeat.
      const rs = new FeedCell(32)
      let j = 0;
    ...
    around (coordinates, d) {
      let t = 0;
      let c = 0;
      for (let x = coordinates.x - d; x < coordinates.x + d; x++) {
        for (let y = coordinates.y - d; y < coordinates.y + d; y++) {
          const r = this.at({ "x": x, "y": y }, 1);
          t += (isNaN(r[0]) ? 0 : r[0]);
          c += (isNaN(r[0]) ? 0 : 1);
        }
      }
    ...
    nearest (coordinates) {
      ...
      let dist = null;
      let r = this.at(coordinates, quantity);
      const r0 = r[0] + (r[0] === 0 ? 1 : 0);
      ...
      for (let x = coordinates.x - d; x < coordinates.x + d; x++) {
        for (let y = coordinates.y - d; y < coordinates.y + d; y++) {
          const o = this.at({ "x": x, "y": y }, quantity);
          if (((((o[0] + (o[0] === 0 ? 1 : 0)) * 103) % 256) < (pAnchor * 256)) &&
              tethered
          ) {
            const d_ = Math.pow(coordinates.x - x, 2) + Math.pow(coordinates.y - y, 2);
    ...
    feedCellAt (coordinates) {
      ...
      let fc = this.cacheFeedCells[cacheKey];
      if (fc == null) {
  ...
  class Anchors {
    constructor (period, probability, radius) {
      ...
      for (let i = 0; i < this.period; i++) {
        for (let j = 0; j < this.period; j++) {
          let closest = 0;
          let dist_ = dist(this.anchors[0].x, this.anchors[0].y, i, j);
          for (let k = 0; k < this.anchors.length; k++) {
            const dist__ = dist(this.anchors[k].x, this.anchors[k].y, i, j);
            if (dist__ < dist_) {
              closest = k;
              dist_ = dist__;
            }
  ...
  class Geometry {
    ...
    gridPlaneCellVisibleMap (func, maxHeight) {
      ...
      for (let i = -2; i < gridHeight + 5 + heightAccountingOffset; i++) {
        for (let j = Math.floor(i / 2); j < gridWidthHalf + 3 + Math.floor(i / 2); j++) {
  ...
  class EffectPlateSquare extends Effect {
    ...
    render (coordinates) {
      ...
      const start = calculated[0];
      const aDist = calculated[1];
      const bDist = calculated[2];
      return (
  ...
  class Cache {
    ...
    refresh () {
      for (let i = 0; i < this.scapes.length; i++) {
        for (const j in this.scapes[i]) {
  ...
  class Render {
    ...
    project (v) {
      ...
      let x = v.coordinates.x;
      let y = v.coordinates.y;
      const z = v.coordinates.z;
    ...
    line (light, look, coordinates, v, g, vA, vB, vC) {
      ...
      const margin =
        (look.line.margin == null) ? (1 / rows) * (mag1 / mag2) : look.line.margin;
      for (let row = 1; row < rows + 1; row++) {
    ...
    prismEnd (look, gs, v, p, scale, offset, color, bottom) {
      const g = this.projection;
      let s = (scale == null) ? { "x": 1, "y": 1 } : scale;
      let o = (offset == null) ? { "x": 0, "y": 0 } : offset;
      ...
      if ((shape.top.y < this.projection.dimensions.height || shape.bot.y > 0) &&
        ...
        let c_ = (color == null) ? look.face.color : color;
    ...
    prismLeft (look, gs, v, p, scale, offset, color) {
      ...
      const s_ = // eslint-disable-line no-unused-vars
        { "x": s.x * v.scales.x, "y": s.y * v.scales.y };
      const o_ = // eslint-disable-line no-unused-vars
        { "x": o.x * v.scales.x, "y": o.y * v.scales.y };
      const yd = (p.dimensions.height * ((1 - v.scales.z) / 2));
    ...
    prism (v) {
      ...
      const hs = [v.dimensions.height, v.dimensions.height, 1];
      const sides = ["prismLeft", "prismRight", "prismTop"];
      const looks = v.looks(true);
      for (let side = 0; side < sides.length; side++) {
        const look = looks[side];
        if (look.pattern == null) {
          ...
          for (let r = 0; r < cols; r++) {
            const hsSide = (hs[side] < 1) ? 1 : hs[side];
            for (let c_ = 0; c_ < rows * hsSide; c_++) {
              const hsSideRatio = (hsSide / Math.ceil(hsSide));
              const c = c_ * hsSideRatio;
              const uY = (sides[side] === "prismTop") ? ((g.uX) * Math.SQRT2) : g.uZ;
              const sX = (sides[side] === "prismLeft")
                ? v.scales.x
                : ((sides[side] === "prismRight") ? (v.scales.y / v.scales.x) : 1);
              const nY = (sides[side] === "prismRight") ? (1 / v.scales.y) : 1;
              this[sides[side]](
                look, g, v, p,
                { "x": 1 / cols, "y": hsSideRatio / (rows * hsSide) },
                {
    ...
    segment (coordinates, length, s, i, v, ry) {
      ry = (ry == null) ? 0 : ry;
      const g = this.projection;
      const x_ = coordinates.x;
      const y_ = coordinates.y;
      const look = v.look;
      const c_ =
        look.emitFaceColor(
          v.feed,
    ...
    cloud (v) {
      ...
      const xSpread = sx * spread;
      const ySpread = sy * spread;
      for (let i = 0; i < v.dimensions.quantity; i++) {
        const df =
        ...
        const dyf = v.feed.randReal([((3 * i) + 1) + df]);
        const dzf = v.feed.randReal([((2 * i) + 1) + df]);
        const dy = dzf * g.uZ * h + ((-g.uY / 2) + (g.uY * dyf));
        const dr = 0.75 + 0.5 * (v.feed.randReal([(2 * i + 2) + df]));
    ...
    prepare (center, ss, fi, cache) {
      ...
      if (ss != null) {
        ...
        const self = this;
        for (let i = 0; i < ss.render().length; i++) {
          ...
          this.projection.gridPlaneCellVisibleMap(function (coordinates) {
            ...
            if (self.effects != null) {
              for (let j = 0; j < self.effects.length; j++) {
      ...
      if (Math.random() > 0.5) {
        vs.sort(function (a, b) {
          ...
          const a_ = a.coordinates, b_ = b.coordinates;
          const do_ = a_.o - b_.o, dz = a_.z - b_.z, dy = a_.y - b_.y, dx = a_.x - b_.x;
    ...
    render (center, ss, fi, precedence, cache) {
      ...
      const vs = this.prepare(center, ss, fi, cache);
      const vps = [];
      if (!precedence) {
    ...
    renderGradually (center, ss, fi, callback, tick, batch, vs, index) {
      ...
      setTimeout(function () {
        for (let i = 0; i < batch; i++) {
          ...
          const v = vs[index];
          const p = self[v.form.shape](v); // eslint-disable-line no-unused-vars
          index += 1;
        }
        if (index < vs.length) {
